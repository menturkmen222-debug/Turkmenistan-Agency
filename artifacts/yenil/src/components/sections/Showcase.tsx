...
export function Showcase() {
  ...
  const testimonials = Array.from({ length: 5 }, (_, i) => ({
    name: t(`showcase.t_${i}_name`),
    business: t(`showcase.t_${i}_business`),
    city: t(`showcase.t_${i}_city`),
    rating: 5,
    text: t(`showcase.t_${i}_text`),
  }));
    ...
    <section className="py-32 bg-surface-2 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 mb-20 text-center">
        <h2 className="text-4xl md:text-6xl font-display font-bold text-white mb-6">
          {t('showcase.testimonials_headline')}
        </h2>
      </div>
          ...
          >
            <div className="flex gap-1 mb-6">
              {[...Array(item.rating)].map((_, j) => (
                <Star key={j} className="w-5 h-5 fill-gold text-gold drop-shadow-[0_0_5px_rgba(212,168,67,0.5)]" />
              ))}
            </div>
            <p className="text-lg text-white mb-8 italic">"{item.text}"</p>
            <div className="flex items-center gap-4 border-t border-white/10 pt-6">
              ...
              <div>
                ...
                <p className="text-muted text-sm">{item.business}, {item.city}</p>
              </div>
            </div>
          </motion.div>
        ))}
      ...
      <div className="max-w-5xl mx-auto px-4 mt-20 text-center relative z-10">
        <h3 className="text-2xl font-bold text-white mb-10">{t('showcase.tech_headline')}</h3>
